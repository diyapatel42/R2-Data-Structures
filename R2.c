 ...
 * ----------------------------------------------------------
 */
unsigned long factorial(int n) {//unsigned long long for 14!
	//recursion function calls itself
	//5! = 5*4*3*2*1
	//base case when will we stop
	if(n < 0){
		printf("Error(factorial): Invalid input\n");
		return 0;
	}//recursion
	if( n == 0 || n == 1)
		return 1;//indent no indent if only one line in if statement
